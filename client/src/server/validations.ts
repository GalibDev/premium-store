...
const recipeBaseSchema = z.object({
  recipeName: z.string().trim().min(3, 'Product name must be at least 3 characters'),
  recipeImage: z.string().trim().url('Product image must be a valid URL'),
  category: z.string().trim().min(2, 'Category is required'),
  cuisineType: z.string().trim().min(2, 'Delivery type is required'),
  difficultyLevel: z.enum(['Instant', 'Manual', 'Scheduled']),
  preparationTime: z.coerce.number().int().positive('Duration must be greater than 0'),
  ingredients: z.array(z.string().trim().min(1)).min(1, 'Add at least one included item'),
