...
type RecipeFormValues = {
  ...
  category: string;
  cuisineType: string;
  difficultyLevel: 'Instant' | 'Manual' | 'Scheduled';
...
export function RecipeForm({ recipe, redirectTo = '/dashboard/recipes' }: { recipe?: Recipe; redirectTo?: string }) {
  ...
  const {
    ...
    defaultValues: {
      ...
      difficultyLevel: recipe?.difficultyLevel || 'Instant',
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="mt-8 grid gap-6 xl:grid-cols-2">
      <div className="dashboard-card space-y-5">
        <label className="block">
          <span className="label-text-strong">Product Name</span>
          <input className="input-clean" {...register('recipeName', { required: true })} />
        </label>
        <label className="block">
          <span className="label-text-strong">Product Image URL</span>
        ...
        {imagePreview ? (
          <img className="h-36 w-full rounded-2xl object-cover" src={imagePreview} alt="Product preview" />
        ) : null}
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="label-text-strong">Delivery Type</span>
      ...
      <div className="dashboard-card space-y-5">
        <div className="grid gap-4 sm:grid-cols-3">
          <label className="block">
            <span className="label-text-strong">Delivery Speed</span>
          ...
          <label className="block">
            <span className="label-text-strong">Duration (days)</span>
        ...
        <label className="block">
          <span className="label-text-strong">What is included (one per line)</span>
          <textarea className="textarea textarea-bordered h-32 w-full" {...register('ingredients', { required: true })} />
        </label>
        <label className="block">
          <span className="label-text-strong">Delivery & activation notes (one per line)</span>
        ...
        <button disabled={isSubmitting} className="btn-brand w-full">
          {editing ? 'Save Product' : 'Publish Product'}
        </button>
      </div>
    </form>
  );
